...
module Maze_generator
    ...
    function points_around(matrix, p, height, width)
        ...
        for i in neighbours
            if 0<i.x<=height && 0<i.y<=width && matrix[i.x,i.y] == 2 # looking for a neighbors
    ...
    function barrier_breaker(matrix, newp, oldp)
        ...
        matrix[x,y] = 1 # call the new point between old and new as a way
    end

    function matrix_generator(height, width)
        """Function generates a matrix of ones and zeros in the form of a maze.
            Arguments:
                height(Int64): matrix height.
                width(Int64): matrix width.
            Return:
                matrix(Array{Int64,2}): matrix of ones and zeros forming a maze.
        """
        matrix = [ 2(i&j&1) for i in 0:height, j in 0:width ] # generate start matrix
        p = Point(2,2) # starting point
        container = [] # conteiner for points 
        ...
        while length(container) != 0 # until the stack is empty
            ...
            newp = points_around(matrix, p, height, width) # take new point
        ...
        matrix[height,width+1] = 1 # exit from the maze
        return matrix
    end

    function maze_image(height, width)
        """Function saves the maze visualization in .png format.
            Arguments:
                height(Int64): matrix height.
                width(Int64): matrix width.
        """
        maze = matrix_generator(height, width) # generate a matrix(maze)
